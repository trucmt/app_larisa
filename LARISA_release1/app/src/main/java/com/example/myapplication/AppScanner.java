        ...
        if (!skipScan) {
            ...
            if (scan_mode.equalsIgnoreCase("custom_scan")) {
                ...
                if (prediction.equalsIgnoreCase(contextRef.get().getString(R.string.safe))) {
                    ...
                    PendingIntent uninstallPendingIntent = PendingIntent.getActivity(contextRef.get(), NOTIFICATION_ID + 1, uninstallIntent, PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_IMMUTABLE);
                    PendingIntent contentIntent1 = PendingIntent.getActivity(contextRef.get(), NOTIFICATION_ID + 2, intent, PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_IMMUTABLE);
                    notificationBuilder.setSmallIcon(R.drawable.ic_notification)
                            .setAutoCancel(true)
